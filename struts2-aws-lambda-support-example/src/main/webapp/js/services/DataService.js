...
(function () {
    ...
    function DataService($http, $log, $q, $rootScope, apiEndpoint) {

        /** Object to manage all backend URL's */
        var urls = {
                orders: apiEndpoint + '/data/order',
                order: apiEndpoint + '/data/order/'
            },
            suffix = "";
        ...
        function _request(url, method, model) {
            var def = $q.defer(),
                req = {
                    method: method,
                    url: addParameterToURL(url + suffix, 'request_only_locale', $rootScope.lang)
                };
...
/*
            if (method === 'GET') {
                req.url = addParameterToURL(req.url, 'request_only_locale', $rootScope.lang);
            } else {
                if(model) {
                    model.request_locale = $rootScope.lang;
                }
            }
*/
            if (model) {
                req.data = JSON.stringify(model);
            }
